...
이 코드를 꼭 넣어주어야 한다. 안 그럼 결코 아예 실행서부터 문제가 생겼다는 메시지 뜨고 기껏 만든 화면 뜨지도 않고 앱이 die… 오늘 한 시간 동안 이걸로 수업이…ㅠㅠ 누군가에게 같은 실수가 반복되어선 안되도록 널리널리 알려야 한다 !! 
...
④ 하나의 파일의 정보가 모두 담긴 music객체를을 보낸다
